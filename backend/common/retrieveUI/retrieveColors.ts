...
import { htmlColorFromFill, htmlGradientFromFills } from "../../html/builderImpl/htmlColor";
import { calculateContrastRatio } from "./commonUI";
import { LinearGradientConversion, SolidColorConversion, Framework } from "types";
import { processColorVariables } from "../../altNodes/jsonNodeConversion";

export const retrieveGenericSolidUIColors = async (framework: Framework): Promise<Array<SolidColorConversion>> => {
...
const convertSolidColor = async (fill: Paint, framework: Framework): Promise<SolidColorConversion | null> => {
					...
					for (const stop of fill.gradientStops) {
						if (stop.boundVariables?.color) {
							try {
								...
								if (variable) {
									(stop as any).variableColorName = variable.name.replace(/\s+/g, "-").toLowerCase();
								}
							} catch (e) {
