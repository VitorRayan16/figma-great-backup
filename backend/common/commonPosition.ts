export const getCommonPositionValue = (node: SceneNode): { x: number; y: number } => {
	...
	// console.log("node parent", !!node.parent, node.parent?.type, node.type, node.parent?.x, node.parent?.y);

	// if (node.parent && (node.parent.type === "GROUP" || node.parent.type === "FRAME")) {
	// 	return {
	...
	// if (!node.absoluteBoundingBox) console.log("não tem absolute", node.name, node.type);
