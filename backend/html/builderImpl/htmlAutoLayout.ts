...
const getFlexDirection = (node: InferredAutoLayoutResult): string => (node.layoutMode === "HORIZONTAL" ? "" : "column");
...
const getGap = (node: InferredAutoLayoutResult): string | number =>
	node.itemSpacing > 0 && node.primaryAxisAlignItems !== "SPACE_BETWEEN" ? node.itemSpacing : "";

const getFlexWrap = (node: InferredAutoLayoutResult): string => (node.layoutWrap === "WRAP" ? "wrap" : "");
...
const getFlex = (node: SceneNode, autoLayout: InferredAutoLayoutResult): string =>
	node.parent && "layoutMode" in node.parent && node.parent.layoutMode === autoLayout.layoutMode
		? "flex"
		: "inline-flex";
