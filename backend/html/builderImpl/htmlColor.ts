...
import { GradientPaint, Paint } from "../../types/figmaApiTypes";
 ...
 */
export const htmlColorFromFills = (fills: ReadonlyArray<Paint> | undefined): string => {
...
export const htmlDiamondGradient = (fill: GradientPaint) => {
	...
	return gradientConfigs
		.map(({ direction, position }) => `linear-gradient(${direction}, ${stops}) ${position} / 50% 50% no-repeat`)
		.join(", ");
};
 ...
 */
export const buildBackgroundValues = (paintArray: ReadonlyArray<Paint> | PluginAPI["mixed"]): string => {
